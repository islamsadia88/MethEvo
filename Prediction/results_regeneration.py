...
def robust_cross_val(x_train, y_train, x_test, y_test, folds):
    ...
    print("Before")
    with open('../Results_regeneration/Whole dataset/Results/RandomForest_d2_t200.pkl', 'wb') as f:
        pickle.dump(model, f)
    print("After")
    res = "{} folds\n".format(folds)
    ...
    with open('../Results_regeneration/Whole dataset/Results/RandomForest_d2_t200.txt', 'a') as f:
        f.write(res)


def load_model(x_train, y_train, x_test, y_test, folds):
    skf = StratifiedKFold(n_splits=folds, random_state=47)
    model = pickle.load(open('../Results_regeneration/Whole dataset/Results/RandomForest_d2_t200.pkl', 'rb'))
...
if __name__ == '__main__':
    npzfile = np.load('../Results_regeneration/Whole dataset/Our model (PSSM+SPD)/whole_data_features.npz', allow_pickle=True)
    X_p = npzfile['arr_0']
    Y_p = npzfile['arr_1']
    X_n = npzfile['arr_2']
    Y_n = npzfile['arr_3']
